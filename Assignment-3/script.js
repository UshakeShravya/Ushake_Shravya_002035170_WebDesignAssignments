...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Function to calculate the next student number dynamically
  const getNextStudentNumber = () => {
      const rows = table.querySelectorAll("tr");
      return rows.length - 1; // Subtract 1 for the header row
  };
  ...
  table.addEventListener("click", (event) => {
      if (event.target.classList.contains("toggle-row")) {
          ...
          event.target.src = detailsRow.classList.contains("hidden") ? "down.png" : "up.png";
      }
  });
  ...
  addButton.addEventListener("click", () => {
      const studentCount = getNextStudentNumber();
      const newRow = `
          <tr>
              <td>
